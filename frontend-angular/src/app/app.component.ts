...
@Component({ standalone: true, imports: [CommonModule, FormsModule], template: `<h1>Cart</h1><div class="alert alert-light border"><p class="mb-1">Cart items are stored locally for the Angular companion.</p><strong>Subtotal: R {{subtotal}}</strong></div><div *ngIf="!token" class="card p-4 mb-3"><h2 class="h5">Sign in before purchase</h2><input class="form-control mb-2" [(ngModel)]="email" placeholder="Email"><input class="form-control mb-2" [(ngModel)]="password" type="password" placeholder="Password"><button class="btn btn-dark" (click)="login()">SIGN IN</button><small class="text-muted mt-2">You can also use the registration route from the API: POST /api/auth/register.</small></div><button class="btn btn-dark" [disabled]="!token" (click)="checkout()">{{token ? "SIMULATE CHECKOUT" : "SIGN IN TO PURCHASE"}}</button>` })
export class CartPageComponent { subtotal = "0.00"; token = localStorage.getItem("nexus-jwt"); email = ""; password = ""; constructor(private http: HttpClient) { const lines = JSON.parse(localStorage.getItem("nexus-cart") ?? "[]"); this.subtotal = lines.reduce((sum: number, line: { price: string; quantity: number }) => sum + Number(line.price) * line.quantity, 0).toFixed(2); } login() { this.http.post<{ token: string }>(`${API}/auth/login`, { email: this.email, password: this.password }).subscribe(result => { this.token = result.token; localStorage.setItem("nexus-jwt", result.token); }); } checkout() { alert("Authenticated simulated checkout ready."); } }
